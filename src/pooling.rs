...
fn max_pool2d_f32(
    ...
    let out_h = if ceil_mode {
        (((h as i64 + 2 * ph - (dh as i64) * (kh as i64 - 1) - 1) as f64 / sh as f64).ceil() as i64 + 1).max(1)
    } else {
        pool_out_size(h as i64, kh as i64, sh as i64, ph, dh as i64)
    };
    let out_w = if ceil_mode {
        (((w as i64 + 2 * pw - (dw as i64) * (kw as i64 - 1) - 1) as f64 / sw as f64).ceil() as i64 + 1).max(1)
    } else {
...
pub fn max_pool2d(
    ...
    let stride = if stride.0 == 0 && stride.1 == 0 { kernel } else { stride };
        ...
        for ci in 0..c {
            ...
            for oh in 0..out_h {
                ...
                for ow in 0..out_w {
                    ...
                    let divisor = if count_include_pad { window } else { count.max(1) };
...
pub fn avg_pool2d(
    ...
    let (sh, sw) = (stride.0 as usize, stride.1 as usize);
    match input.dtype {
        DType::F32 => avg_pool2d_f32(input, (kh, kw), (sh, sw), padding, ceil_mode, count_include_pad),
        DType::F64 => avg_pool2d_f64(input, (kh, kw), (sh, sw), padding, ceil_mode, count_include_pad),
...
fn adaptive_avg_pool2d_f32(input: &BorrowedTensor, out_h: usize, out_w: usize) -> PyResult<OwnedTensor> {
    let b = input.shape[0] as usize;
    let c = input.shape[1] as usize;
    let h = input.shape[2] as usize;
    let w = input.shape[3] as usize;
...
fn adaptive_avg_pool2d_f64(input: &BorrowedTensor, out_h: usize, out_w: usize) -> PyResult<OwnedTensor> {
    let b = input.shape[0] as usize;
    let c = input.shape[1] as usize;
    let h = input.shape[2] as usize;
    let w = input.shape[3] as usize;
...
pub fn adaptive_avg_pool2d(
    ...
    if input.shape.len() != 4 {
        return Err(unsupported("adaptive_avg_pool2d: input must be 4-D (B,C,H,W)"));
    }
        ...
        Some(v) => {
            if let Some(s) = v.as_i64() {
                ...
                match vals.len() {
                    1 => (vals[0], vals[0]),
                    2 => (vals[0], vals[1]),
                    _ => return Err(unsupported("adaptive_avg_pool2d: output_size must be an int or pair")),
                }
            } else {
                return Err(unsupported("adaptive_avg_pool2d: output_size must be an int or pair"));
            }
        }
    };
    if out_h <= 0 || out_w <= 0 {
        return Err(unsupported("adaptive_avg_pool2d: output_size must be positive"));
    }
    match input.dtype {
...
fn adaptive_max_pool2d_f32(input: &BorrowedTensor, out_h: usize, out_w: usize) -> PyResult<OwnedTensor> {
    let b = input.shape[0] as usize;
    let c = input.shape[1] as usize;
    let h = input.shape[2] as usize;
    let w = input.shape[3] as usize;
...
fn adaptive_max_pool2d_f64(input: &BorrowedTensor, out_h: usize, out_w: usize) -> PyResult<OwnedTensor> {
    let b = input.shape[0] as usize;
    let c = input.shape[1] as usize;
    let h = input.shape[2] as usize;
    let w = input.shape[3] as usize;
...
pub fn adaptive_max_pool2d(
    ...
    if input.shape.len() != 4 {
        return Err(unsupported("adaptive_max_pool2d: input must be 4-D (B,C,H,W)"));
    }
    let (out_h, out_w) = match output_size {
        None => (1, 1),
        Some(v) => {
            if let Some(s) = v.as_i64() {
                ...
                match vals.len() {
                    1 => (vals[0], vals[0]),
                    2 => (vals[0], vals[1]),
                    _ => return Err(unsupported("adaptive_max_pool2d: output_size must be an int or pair")),
                }
            } else {
                return Err(unsupported("adaptive_max_pool2d: output_size must be an int or pair"));
            }
        }
    };
    if out_h <= 0 || out_w <= 0 {
        return Err(unsupported("adaptive_max_pool2d: output_size must be positive"));
    }
    match input.dtype {
...
fn max_pool1d_f32(input: &BorrowedTensor, kernel: usize, stride: usize, padding: i64, ceil_mode: bool) -> PyResult<OwnedTensor> {
    let b = input.shape[0] as usize;
    let c = input.shape[1] as usize;
    let l = input.shape[2] as usize;
    let out_l = if ceil_mode {
        (((l as i64 + 2 * padding - kernel as i64 - 1) as f64 / stride as f64).ceil() as i64 + 1).max(1)
    } else {
...
fn max_pool1d_f64(input: &BorrowedTensor, kernel: usize, stride: usize, padding: i64, ceil_mode: bool) -> PyResult<OwnedTensor> {
    let b = input.shape[0] as usize;
    let c = input.shape[1] as usize;
    let l = input.shape[2] as usize;
    let out_l = if ceil_mode {
...
fn avg_pool1d_f32(input: &BorrowedTensor, kernel: usize, stride: usize, padding: i64, ceil_mode: bool, count_include_pad: bool) -> PyResult<OwnedTensor> {
    let b = input.shape[0] as usize;
    let c = input.shape[1] as usize;
    let l = input.shape[2] as usize;
    let out_l = if ceil_mode {
    ...
    out_data.par_chunks_mut(plane_elems).enumerate().for_each(|(bi, out_plane)| {
        for ci in 0..c {
            ...
            for ol in 0..out_l {
                ...
                let divisor = if count_include_pad { kernel } else { count.max(1) };
...
fn avg_pool1d_f64(input: &BorrowedTensor, kernel: usize, stride: usize, padding: i64, ceil_mode: bool, count_include_pad: bool) -> PyResult<OwnedTensor> {
    let b = input.shape[0] as usize;
    let c = input.shape[1] as usize;
    let l = input.shape[2] as usize;
    let out_l = if ceil_mode {
...
pub fn avg_pool1d(
    ...
    let padding = scalar(padding, "padding", 0)?;
    match input.dtype {
        DType::F32 => avg_pool1d_f32(input, kernel as usize, stride as usize, padding, ceil_mode, count_include_pad),
        DType::F64 => avg_pool1d_f64(input, kernel as usize, stride as usize, padding, ceil_mode, count_include_pad),
